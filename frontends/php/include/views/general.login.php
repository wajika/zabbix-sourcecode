...
$brand_logo = CBrandHelper::getLogo();
$brand_logo_tag = ($brand_logo !== false) ? new CImg($brand_logo) : (new CSpan())->addClass(ZBX_STYLE_ZABBIX_LOGO);
...
(new CDiv([
	(new CTag('main', true, [
		(isset($ZBX_SERVER_NAME) && $ZBX_SERVER_NAME !== '')
			...
			: null,
		(new CDiv([
			(new CDiv($brand_logo_tag))->addClass(ZBX_STYLE_SIGNIN_LOGO),
			(new CForm())
				->cleanItems()
