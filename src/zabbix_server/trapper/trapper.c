 ...
 *                                                                            *
 ******************************************************************************/
static int	recv_media_query(zbx_socket_t *sock, struct zbx_json_parse *jp)
{
	...
	zbx_json_free(&json);
clean:
	zbx_vector_ptr_clear_ext(&tickets, (zbx_mem_free_func_t)zbx_free_ticket);
 ...
 *                                                                            *
 ******************************************************************************/
static int	recv_media_acknowledge(zbx_socket_t *sock, struct zbx_json_parse *jp)
{
	const char		*__function_name = "recv_media_query";
	int			ret = FAIL;
...
clean:
	...
	zbx_vector_ptr_clear_ext(&acknowledges, (zbx_mem_free_func_t)zbx_free_acknowledge);
